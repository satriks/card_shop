...
def send_new_order_notification(order_id):
    # Импортируйте вашу модель заказа
    from orders.models import Order  # Замените на правильный импорт вашей модели заказа
    order = Order.objects.get(pk=order_id)  # Получаем заказ по ID

    # Предполагается, что у вас есть адрес электронной почты владельца сайта
    ...
    plain_message = strip_tags(html_message)
    from_email = 'satriks@mail.ru'  # Или используйте другой адрес электронной почты
    to = owner_email
    email = EmailMultiAlternatives(subject, plain_message, from_email, [to])
    email.content_subtype = "html"  # Указываем, что это HTML-сообщение
    email.attach_alternative(html_message, "text/html")
